...
export default function AdminProductList({
    ...
    render: (value, item) => (
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          <p className="font-semibold text-gray-900 truncate text-sm">{value}</p>
          <p className="text-xs text-gray-500 truncate">{item.category?.name || 'Uncategorized'}</p>
        </div>
      </div>
    ),
    className: 'w-64',
  ...
  // Conditionally add Stock column
  if (showStock) {
    columns.push({
      key: 'stock',
      header: 'Stock',
      render: (value) => {
        ...
        return (
          <div className="flex justify-center">
            <span className={`badge badge-sm font-semibold ${badgeClass}`}>{value} units</span>
          </div>
        );
      },
